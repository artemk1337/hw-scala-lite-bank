...
import misis.model.{AccountUpdate, FeeRequest}
...
class FeeStreams(repository: FeeRepository)(implicit val system: ActorSystem, executionContext: ExecutionContext)
    ...
    kafkaSource[FeeRequest]
        .mapAsync(1) { command =>
            val limit: Int = repository.updateLimit(command.srcAccountId, command.money)

            if (limit < 0) {
                val fee = repository.getFee(command.money)
                println(
